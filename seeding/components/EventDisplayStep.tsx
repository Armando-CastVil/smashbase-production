...
export default function EventDisplayStep({
  page,
  setPage,
  apiKey,
  events,
  ...
  const handleSubmit = async () => {
    ...
    //if a checked box was found, go through the submission motions
      ...
      setPhaseGroups(
        returnPhaseGroupArray(await getPhaseGroupWrapper(instantSlug, apiKey!))
      );
...
async function getPhaseGroup(slug: string, apiKey: string) {
  //API call
  const { data } = await axios.get("api/getPhaseGroup", {
    params: { slug: slug, apiKey: apiKey },
  });
  return data;
}
